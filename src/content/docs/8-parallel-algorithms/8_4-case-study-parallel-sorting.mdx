...
Time Complexity: O((n/p)log(n/p) + n log p) = O((n log n)/p + n log p)
Communication: O(log p) merge phases
...
Total depth: log²(n) comparisons
Parallelism: All comparisons at same distance can execute simultaneously
...
parallel for each processor i = 0 to p-1 do
    ...
    for j = 1 to samples_per_processor do
        // Random sampling from local data
        random_index = random(0, local_data_size - 1)
        local_samples[i].append(A[i][random_index])
    end
end parallel

// Phase 2: Gather and select splitters
all_samples = gather(local_samples)  // p * samples_per_processor samples
sort(all_samples)

// Select p-1 evenly spaced splitters
splitters[1..p-1] = []
for k = 1 to p-1 do
    index = k * samples_per_processor * oversampling_factor
    splitters[k] = all_samples[index]
end

// Phase 3: Broadcast splitters to all processors
broadcast(splitters)

// Phase 4: Local partitioning into buckets
parallel for each processor i = 0 to p-1 do
    buckets[i][0..p-1] = initialize_empty_buckets()

    for each element in local_data[i] do
...
// Phase 5: All-to-all exchange (redistribution)
parallel for each processor i = 0 to p-1 do
    new_local_data[i] = []

    // Send bucket j from processor i to processor j
    for j = 0 to p-1 do
        send(buckets[i][j], to_processor=j)
    end

    // Receive bucket i from all processors
    for j = 0 to p-1 do
        received = receive(from_processor=j)
        new_local_data[i].append(received)
    end
end parallel
...
// Result: Concatenation of sorted data from processor 0 to p-1
return concatenate(new_local_data[0], ..., new_local_data[p-1])
```

Time Complexity:
- Sampling: O(samples_per_processor)
- Local sort: O((n/p) log(n/p))
- All-to-all: O(n/p) communication
- Total: O((n/p) log(n/p) + n/p)
