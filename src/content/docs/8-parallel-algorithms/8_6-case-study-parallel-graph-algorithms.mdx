...
```pseudo
Algorithm: DeltaStepping_SSSP(Graph G, source s, Δ, p processors)
...
// Buckets organize vertices by distance ranges [i*Δ, (i+1)*Δ)
...
while exists non-empty bucket do
    ...
    // Relax heavy edges (weight > Δ) once per bucket
