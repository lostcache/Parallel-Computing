...
Total levels: O(diameter of graph)
Synchronization: O(diameter) barriers
...
*   **Delta-Stepping Algorithm:** This is a widely used parallel SSSP algorithm. Instead of processing one vertex at a time, it processes vertices in "buckets," where each bucket corresponds to a range of distances of size Δ. In each phase, all "light" edges (those with weight ≤ Δ) connected to vertices in the current lowest-indexed non-empty bucket can be relaxed in parallel. This may introduce redundant work, but it exposes significant parallelism by allowing many vertices to be processed concurrently.[^89]
...
Input: Weighted graph G = (V, E), source s, parameter Δ, p processors
...
// Buckets: bucket[i] contains vertices with tentative distance in [i*Δ, (i+1)*Δ)
...
while exists non-empty bucket do
    ...
    while buckets[current_bucket_index] is empty do
        current_bucket_index = current_bucket_index + 1
    end

    // Process current bucket until empty
    while buckets[current_bucket_index] is not empty do
        current_set = buckets[current_bucket_index]
        buckets[current_bucket_index] = empty

        // Phase 1: Relax light edges (weight ≤ Δ) in parallel
        parallel for each vertex v in current_set using p processors do
            for each light edge (v, u) with weight w ≤ Δ do
                new_dist = dist[v] + weight(v, u)

                if new_dist < dist[u] then
                    // Atomic update of distance
                    old_dist = atomic_min(dist[u], new_dist)

                    if old_dist > new_dist then
                        // Remove u from old bucket (if present)
                        old_bucket = floor(old_dist / Δ)
                        buckets[old_bucket].remove(u)

                        // Add u to new bucket
                        new_bucket = floor(new_dist / Δ)
                        buckets[new_bucket].add(u)
                    end
                end
            end
        end parallel
    ...
    // Phase 2: Relax heavy edges (weight > Δ) once per bucket
    parallel for each vertex v that was in current bucket do
        for each heavy edge (v, u) with weight w > Δ do
            new_dist = dist[v] + weight(v, u)
...
Time Complexity: O((|V| + |E|/Δ) × log|V|) expected
Parameter Δ: Trade-off between parallelism and redundant work
- Small Δ: More buckets, less redundant work, less parallelism
- Large Δ: Fewer buckets, more redundant work, more parallelism
...
while num_components > 1 do
    ...
    parallel for each vertex v in V using p processors do
        ...
        for each edge (v, u) incident to v do
            if component[v] ≠ component[u] and weight(v, u) < min_weight then
                min_edge = (v, u)
    ...
    MST_edges = MST_edges ∪ selected_edges
    barrier()
...
Time Complexity: O(log |V|) iterations × O(|E|/p) per iteration
