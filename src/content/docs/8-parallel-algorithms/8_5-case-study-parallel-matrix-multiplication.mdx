...
Matrix-matrix multiplication is a cornerstone of scientific and engineering computation, forming the computational core of applications in fields from linear algebra and physics simulations to machine learning. Its high computational intensity, with O(n³) arithmetic operations for n×n matrices, makes it an excellent candidate for parallelization and a standard benchmark for high-performance computing (HPC) systems.[^69] This case study examines two classic algorithms for distributed-memory parallel matrix multiplication, Cannon's algorithm and SUMMA, which employ distinct data decomposition and communication patterns to distribute the workload across a 2D grid of processors.

### **Cannon's Algorithm**

Cannon's algorithm is a memory-efficient parallel algorithm for multiplying two matrices on a 2D processor grid, typically a square grid of p = √p × √p processors with a torus (wraparound) network topology.[^71] It orchestrates the movement of data blocks in a highly structured, nearest-neighbor fashion to ensure each processor computes its share of the result matrix.

#### **Methodology**

The algorithm for computing C = AB on a √p × √p grid, where processor P(i,j) is responsible for computing the sub-block C(i,j), proceeds in two main phases:[^71]
...
1.  **Initial Alignment:** Before the main computation begins, the input matrices A and B are pre-aligned.
    ...
    *   For each column j, the block B(i,j) is circularly shifted up by j positions.
    This initial skew ensures that after alignment, processor P(i,j) holds blocks A(i, (j+i)%√p) and B((i+j)%√p, j). These are the blocks needed to compute the first term in the sum for C(i,j).
...
```pseudo
Algorithm: CannonsAlgorithm(A[n×n], B[n×n], √p × √p processor grid)
Input: Matrices A and B of size n×n, p = √p × √p processors
Output: Matrix C = A × B
...
parallel for each processor P(i, j) do
    ...
    A_local = receive_from(P(i, (j+i) mod √p))

    // Shift B blocks: column j shifted up by j positions (circular)
    B_local = receive_from(P((i+j) mod √p, j))

    // Initialize result block
    C_local = 0
end parallel

// Phase 2: Compute-and-Shift Loop (√p iterations)
for step = 0 to √p - 1 do
    parallel for each processor P(i, j) do
        // Compute: multiply local blocks and accumulate
        C_local = C_local + (A_local × B_local)

        // Shift A blocks: send left, receive from right
        send(A_local, to_processor=P(i, (j-1) mod √p))
        A_local = receive_from(P(i, (j+1) mod √p))

        // Shift B blocks: send up, receive from below
        send(B_local, to_processor=P((i-1) mod √p, j))
        B_local = receive_from(P((i+1) mod √p, j))
    end parallel
end
...
Time Complexity:
- Computation: O(√p) steps × O((n/√p)³) = O(n³/p)
- Communication: O(√p) nearest-neighbor shifts
- Memory per processor: O(n²/p)
...
```pseudo
Algorithm: SUMMA(A[n×n], B[n×n], p_rows × p_cols processor grid)
Input: Matrices A and B of size n×n, processor grid p_rows × p_cols
Output: Matrix C = A × B

// Processor P(i,j) is responsible for computing block C(i,j)
// A is distributed by rows, B by columns

block_rows = n / p_rows
block_cols = n / p_cols
block_size = n / √p  // For simplicity, assume square blocks
...
// Outer loop: iterate over panels
num_panels = n / block_size

for k = 0 to num_panels - 1 do
    parallel for each processor P(i, j) do
        // Broadcast panel of A horizontally
        if j == owner_of_column_k then
            // This processor owns the k-th block column of A
            A_panel = A_local[i][k]
        end

        // Broadcast A_panel to all processors in row i
        A_panel = broadcast_along_row(A_panel, row=i)

        // Broadcast panel of B vertically
        if i == owner_of_row_k then
            // This processor owns the k-th block row of B
            B_panel = B_local[k][j]
        end

        // Broadcast B_panel to all processors in column j
        B_panel = broadcast_along_column(B_panel, column=j)

        // Rank-k update: local computation
        C_local[i][j] = C_local[i][j] + (A_panel × B_panel)
    end parallel
end
...
Example for 4×4 processors:
- Iteration 0: Broadcast column 0 of A, row 0 of B
  - P(0,0) computes: C(0,0) += A(0,0) × B(0,0)
  - P(1,2) computes: C(1,2) += A(1,0) × B(0,2)
- Iteration 1: Broadcast column 1 of A, row 1 of B
  - Continue accumulating partial products

Time Complexity:
- Computation: O(n³/p)
- Communication: O(k/b × n²/√p) broadcasts where k is matrix dimension
- Memory per processor: O(n²/p)
...
*   **Memory:** Both are "2D" algorithms, meaning they distribute a single copy of the matrices over a 2D processor grid and have a memory footprint of O(n²/p) per processor. Under this constraint, their communication costs are asymptotically optimal.[^77]

This analysis leads to a deeper principle in modern HPC: trading memory for communication. The existence of "2.5D" and "3D" matrix multiplication algorithms demonstrates this trade-off. A 3D algorithm, for instance, might arrange p processors in a p¹ᐟ³×p¹ᐟ³×p¹ᐟ³ cube and replicate the input matrices p¹ᐟ³ times along the third dimension. This extra memory usage allows much of the required data to be local, dramatically reducing the volume of data that must be communicated over the network. Using c copies of the data can reduce the communication bandwidth cost by a factor of c¹ᐟ² and the latency cost by c³ᐟ².[^78] This shows that the notion of an "optimal" algorithm is not absolute but is contingent on available resources. While 2D algorithms like Cannon's and SUMMA are optimal under minimal memory constraints, communication-avoiding algorithms like the 2.5D/3D variants can achieve superior performance if additional memory is available.
